...
class Twitter:
    ...
    def scrap(username,browser_name):
        try:
            ...
            driver.close()
            driver.quit()
            #print(type(json.dumps(profile_data)))
            return(profile_data)
...
if __name__ == '__main__':
    ...
    for username in uname:
        aa=Twitter.scrap(username,browser_name)
        print(type(Twitter.scrap(username,browser_name)))
...
#df = pd.DataFrame(['aa']).to_excel("abc.xlsx")    
#df = pd.read_json(aa).to_excel("aa.xlsx")
# with open("sample.json","w" ) as opfile:
#     json.dump(aa, opfile) 
# with open('sample.json', 'r')  as opfile:
#     json_object = json.load(opfile) 
# print(json_object)
# df = pd.read_json("sample.json").to_excel("aa.xlsx")  
    #print(json2excel.run(aa))
        # list1=list(aa.items())
        # print(list1)
        # df=pd.DataFrame(data=list1,index=[1])
